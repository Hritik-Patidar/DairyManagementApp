...
def setup_database():
    ...
    """)
    cursor.execute("""
    INSERT OR IGNORE INTO FatCLRRate (Fat, CLR, Rate) VALUES
    ...
    # milk_types = ["C", "B"]
    #
    # # 🧩 Farmer table insert with HindiName
    # for acc_no in range(1, 1501):
    #     name = random.choice(names)
    #     hindi_name = name_map.get(name, name)  # fallback to English if missing
    #     phone = f"9{random.randint(100000000, 999999999)}"
    #     milk_type = random.choice(milk_types)
    #
    #     cursor.execute("""
    #         INSERT OR IGNORE INTO farmers (account_no, name, HindiName, phone, milk_type)
    #         VALUES (?, ?, ?, ?, ?)
    #     """, (acc_no, name, hindi_name, phone, milk_type))
    #
    # print("✅ Dummy farmers inserted successfully (with Hindi names).")
    #
    # # 🥛 DailyEntry generation
    # today = datetime.now().date()
    # for day_offset in range(600):  # last 600 days
    #     entry_date = (today - timedelta(days=day_offset)).strftime("%Y-%m-%d")
    #     for acc_no in range(1, 1501):
    #         milk_type = random.choice(["C", "B"])
    #         for shift in ["M", "E"]:
    #             qty = round(random.uniform(1, 10), 2)
    #             fat = round(random.uniform(3.5, 5.5), 1)
    ...
    #
    # print("Dummy daily entries inserted successfully.")
    conn.commit()
    conn.close()
